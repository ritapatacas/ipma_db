...
function buildForecastCardView() {
  ...
  const iconNow = firstDefined(currentRow, ["icon"]) || day.icon || '<i class="fa-solid fa-cloud"></i>';
  ...
  return (
    ...
    `</div>` +
    `</div>` +
    `<div class="forecast-hero-right"><h3>Weather</h3><p>${escapeHtml(String(day.weekday || ""))} ${escapeHtml(String(nowTime))}</p><p>${escapeHtml(String(description))}</p></div>` +
    `</header>` +
    ...
    `<button class="forecast-week-arrow" data-forecast-nav="prev" aria-label="Semana anterior" ${prevDisabled}>` +
    `<i class="fa-solid fa-chevron-left" aria-hidden="true"></i>` +
    `</button>` +
    `<div class="daily-cards-row">${cards}</div>` +
    `<button class="forecast-week-arrow" data-forecast-nav="next" aria-label="Semana seguinte" ${nextDisabled}>` +
...
function renderForecastChart() {
  ...
  window.forecastHourlyChart = new Chart(canvas.getContext("2d"), {
    ...
    options: {
      ...
      scales: {
        x: { grid: { display: false, drawBorder: false }, ticks: { autoSkip: true, maxTicksLimit: 8, color: "#8f949b", maxRotation: 0 } },
        y: { beginAtZero: state.metric !== "temperature", grid: { color: "rgba(30, 41, 59, 0.08)", drawBorder: false }, ticks: { color: "#8f949b", maxTicksLimit: 5 } },
      },
    },
  });
}
