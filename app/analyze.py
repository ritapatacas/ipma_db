...
def _load_weather_data() -> pd.DataFrame:
    ...
    try:
        ...
        return df[keep_columns].dropna(subset=['datetime']).sort_values('datetime')
...
def _create_time_periods(df: pd.DataFrame, group_by: str) -> pd.DataFrame:
    ...
    df = df.copy()
    if group_by == "week":
        df['period'] = df['datetime'] - pd.to_timedelta((df['datetime'].dt.weekday) % 7, unit='D')
    elif group_by == "month":
...
def print_data_table(df: pd.DataFrame, title: str, date_col: str = 'date', time_col: str = 'time') -> None:
...
def summarize_cold_hours(group_by: str = "date") -> pd.DataFrame:
    ...
    summary = cold_hours.groupby('period', as_index=False).size().rename(columns={'size': 'cold'})
...
def summarize_missing_entries(group_by: str = "date") -> pd.DataFrame:
    ...
    missing['missing'] = (missing['expected'] - missing['actual']).astype(int)  # Force integer type
    
    missing = _create_time_periods(missing.rename(columns={'date': 'datetime'}), group_by)
...
def print_summary_table(summary_df: pd.DataFrame, title: str, value_col: str, group_by: str = "date") -> None:
